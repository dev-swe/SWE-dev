...
# ==================== CONFIG ====================
# config_loader walks up the directory tree from __file__ to find config.py,
# adds lib/ to sys.path as a side-effect, and returns the config namespace.
# This replaces the former inline SCRIPT_DIR / EXTENSION_DIR / _load_config()
# block that was copy-pasted across scripts.
...
if not FALLBACK_SP_FILE:
    ...
    _pf.alert('SP_FILE is missing in config.py', exitscript=True)
...
def get_project_shared_parameter_file(app, prompt_user=False):
    ...
    if FALLBACK_SP_FILE and os.path.exists(FALLBACK_SP_FILE):
...
def _get_first_planar_face_and_edge(fam_doc):
    ...
    # PERF: lazy iteration — short-circuits without materialising every element.
...
class FamilyItemVM(BaseViewModel):
    ...
    @property
    def FamilyElem(self):   return self._elem
    @property
    ...
    def update_elem(self, new_elem):
        ...
        Mutating in place keeps the same Python object reference in the bound
        ListBox, preserving the user's selection without code-behind work.
        """
        self._elem = new_elem
...
class MainViewModel(BaseViewModel):
    ...
    The View binds to properties and commands here via DataContext.
    The View layer has no business logic and no Revit imports.
    """
    ...
    @property
    def FamilyItems(self):       return self._family_items
    @property
    def Categories(self):        return self._categories
    @property
    def ParameterRows(self):     return self._param_rows
    @property
    def ConnectorTypes(self):    return self._connector_types
    @property
    def StatusText(self):        return self._status_text
    @property
    def ResultText(self):        return self._result_text
    @property
    def ResultDetailsText(self): return self._result_details_text
    @property
    def HasResultDetails(self):  return bool(self._result_details_text)
    @property
    def ResultBrush(self):       return '#B91C1C' if self._result_is_error else '#12413C'
    @property
    def ResultVisible(self):     return bool(self._result_text)
    @property
    def ActiveCountText(self):   return self._active_count_text
    @property
    def FamilyCountText(self):   return '({0})'.format(len(self._family_items))
    @property
    def IsStepperEnabled(self):  return self._is_multipoint
    @property
    def ConnectionCountText(self): return str(self._connection_count)
    ...
    def set_selected_family(self, item):
        ...
        self._notify('SelectedFamilyItem', 'SelectedFamilyName',
                     'SelectedFamilyCategory', 'HasCategoryBadge')
    ...
    def _apply_family_filter(self):
        ...
        filtered = (
            ...
            else [fi for fi in self._family_all if fi.CategoryName == cat]
        )
        # PERF: build a fresh ObservableCollection so the ListBox refreshes once.
        ...
        for name in ordered:
            ...
            if data is None:
                row = SharedParameterRowVM(
                    ...
                    description='', guid='', not_found=True, parent_vm=self,
                )
            else:
                row = SharedParameterRowVM(
                    param_name=name,
    ...
    def _execute_auto_add(self):
        ...
        # Refresh tray against the reloaded family element.
        refreshed = None
        try:
        ...
        if assoc_results: summary_parts.append('Associated {0}'.format(len(assoc_results)))
        summary_parts.append('Connector: {0} ({1})'.format(conn_msg, conn_label))
        self._set_result(' | '.join(summary_parts), is_error=bool(failed))
